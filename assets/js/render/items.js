window.RenderItems = (function(){
  ...
  function itemTooltip(item){
    ...
    return `<div class="tooltip-head"><img src="${item.image}" alt=""><div><div class="tooltip-title">${AppUtils.displayName(item)}</div><div class="tooltip-sub">${AppUtils.rarityName(item.rarity)}</div></div></div>
      ...
      ${sections.stats.length ? sectionBlock(AppUtils.lang()==='es' ? 'Stats y scaling' : 'Stats and scaling', statGrid(sections.stats), 'stats compact') : ''}
  ...
  function toggleDataSet(attrName){
    return event => {
      ...
      const map = {
        rarity:'selectedRarities', role:'selectedRoles', range:'selectedRanges', attr:'selectedAttrs'
      };
