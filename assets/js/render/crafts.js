window.RenderCrafts = (function(){
  ...
  function normalizeRecipes(){
    ...
    values.sort((a,b)=>{
      ...
      return rarityDiff || a.result.localeCompare(b.result);
    });
    return values;
  }
  ...
  function formulaBlock(craft, formula, index){
  ...
  function openModal(resultKey){
    ...
    document.getElementById('craftModalBody').innerHTML = `<button class="btn modal-close" id="craftCloseBtn">×</button>${craft.formulas.map((formula, index)=>formulaBlock(craft, formula, index)).join(craft.formulas.length>1?'<div class="recipe-divider"></div>':'')}`;
  ...
  function render(){
    ...
    const filtered = window.AppRecipes.filter(craft => AppUtils.searchMatch(`${craft.result} ${craft.formulas.map(formula => formula.components.join(' ')).join(' ')}`, AppState.craftSearch));
    const groups = groupedCrafts(filtered);
    root.innerHTML = `<div class="section-card"><div style="display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap"><div class="count-badge">${filtered.length} crafts</div><div style="display:flex;gap:8px"><button class="btn" id="expandAllCrafts">${AppUtils.t('expandAll')}</button><button class="btn" id="collapseAllCrafts">${AppUtils.t('collapseAll')}</button></div></div>${groups.map(group => `<section class="craft-group" data-group="${group.rarity}"><div class="item-group-head clickable-head"><div class="count-badge">${AppUtils.rarityName(group.rarity)}</div><div style="display:flex;gap:10px;align-items:center"><div class="count-badge">${group.crafts.length} crafts</div><button class="btn craft-group-toggle" data-group="${group.rarity}" type="button">–</button></div></div><div class="craft-grid">${group.crafts.map(craft => `<article class="craft-card r${craft.rarity} hover-craft" data-result-key="${craft.key}"><div class="craft-thumb">${craft.image?`<img src="${craft.image}" alt="${craft.result}">`:''}</div><div class="craft-name">${craft.result}</div><div class="craft-meta">${craft.formulas.length} recipe${craft.formulas.length===1?'':'s'}</div></article>`).join('')}</div></section>`).join('')}</div><div class="modal-backdrop" id="craftBackdrop"></div><div class="modal" id="craftModal"><div id="craftModalBody"></div></div>`;
