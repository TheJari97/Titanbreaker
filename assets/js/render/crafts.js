window.RenderCrafts = (function(){
  ...
  function craftTooltip(craft){
    ...
    return `<div class="tooltip-head"><img src="${craft.image||''}" alt=""><div><div class="tooltip-title">${craft.result}</div><div class="tooltip-sub">${AppUtils.rarityName(craft.rarity)}</div></div></div><div class="tooltip-section tooltip-section-profile"><div class="tooltip-section-title">${AppUtils.lang()==='es'?'Perfil de receta':'Recipe profile'}</div><div class="tooltip-badge-row"><span class="mini-badge role-badge role-${craft.role}">${AppUtils.roleName(craft.role)}</span><span class="mini-badge filter-badge filter-range">${AppUtils.rangeName(craft.range)}</span><span class="mini-badge filter-badge filter-attr">${AppUtils.attrName(craft.attr)}</span></div></div><div class="tooltip-section tooltip-section-craft"><div class="tooltip-section-title">${AppUtils.lang()==='es'?'Componentes base':'Base components'}</div><ul>${componentsPreview}</ul></div>`;
  }
