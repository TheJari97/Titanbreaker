window.AppUtils = (function(){
  ...
  function aliasName(name){ const raw=String(name||'').trim(); if(typeof RECIPE_ALIASES!=='undefined' && RECIPE_ALIASES[raw]) return RECIPE_ALIASES[raw]; return raw; }
  function recipeDisplayName(name){ return sanitizeName(aliasName(name)); }
  ...
  function itemActs(item){ return []; }
  function itemKey(item){ return item ? (item.codename || normalizeKey(displayName(item))) : ''; }
  function findItemByName(name){
    const raw = String(name||'').trim();
    if(!raw) return null;
    const aliased = aliasName(raw);
    const attempts = [raw, aliased, recipeDisplayName(raw), stripBrackets(raw), stripQualityWords(raw), sanitizeName(raw), sanitizeName(aliased)].filter(Boolean);
  ...
  return { rarityOrder, rarityNames, roleOrder, roleNames, rangeOrder, rangeNames, attrOrder, attrNames, lang, t, stripBrackets, stripQualityWords, sanitizeName, normalizeKey, descByLang, displayName, aliasName, recipeDisplayName, rarityName, roleName, rangeName, attrName, searchMatch, itemRoles, itemRange, itemAttribute, itemActs, itemKey, findItemByName, maxSkillRank, cloneSet, setIsAll, filterIsInactive, matchesFilter };
})();
